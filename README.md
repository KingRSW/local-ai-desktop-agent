...
- 右上「cpu」模型页：看当前后端 + 可下载离线 GGUF 模型（推理引擎接入中，规划路线2）。下载源走 `hf-mirror.com` 国内镜像（huggingface.co 直连被墙会超时），含 Qwen2.5-0.5B/1.5B/3B、Llama-3.2-3B 等；失败会红字提示不再静默。注：Gemma 等在 HF 属需登录的门控模型，无法在 App 内匿名下载，故未列入。
